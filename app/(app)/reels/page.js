...
// User Video Reel Card Component (for Go Live Lite recordings)
function UserVideoReelCard({ reel, isActive, onSaveToggle, onReact, onDelete, isMuted, onMuteToggle }) {
  const config = USER_VIDEO_CONFIG
  const [saved, setSaved] = useState(false)
  const [isPlaying, setIsPlaying] = useState(false)
  const [videoError, setVideoError] = useState(false)
  const [showControls, setShowControls] = useState(true)
  ...
  useEffect(() => {
    setSaved(isSaved('user_video', reel.id))
  }, [reel.id])
  ...
  const handleVideoError = () => {
    console.error('[UserVideoReelCard] Video failed to load:', reel.videoUrl)
    setVideoError(true)
  }
      ...
      <div className="relative w-full max-w-lg h-full max-h-[600px] flex flex-col">
        ...
        <div className={`flex-1 rounded-3xl bg-gradient-to-br ${config.gradient} backdrop-blur-xl border ${config.borderAccent} shadow-2xl overflow-hidden flex flex-col`}>
          ...
          <div className="p-4 flex items-center justify-between">
            ...
            <span className="px-2 py-1 rounded-full bg-orange-500/30 text-orange-300 text-xs font-medium">
              🎥 Video
            </span>
          </div>
          ...
          <div className="flex-1 relative bg-black/30">
            {videoError ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center">
                ...
                <h3 className="text-white font-semibold mb-2">Video unavailable</h3>
                <p className="text-gray-400 text-sm mb-4">This video is no longer available</p>
                <button
                  onClick={(e) => { e.stopPropagation(); setVideoError(false); }}
                  className="px-4 py-2 rounded-lg bg-orange-500/20 text-orange-400 border border-orange-500/40 text-sm font-medium hover:bg-orange-500/30 transition-colors"
                >
                  <RefreshCw className="w-4 h-4 inline mr-2" />
                  Retry
                </button>
...
export default function ReelsPage() {
  ...
  // Delete AI-generated or user video reel
  const handleDeleteAiReel = async (reelId) => {
    try {
      ...
      const isAiVideo = reelToDelete?.isAiVideo || reelToDelete?.type === 'ai_video'
      const isUserVideo = reelToDelete?.isUserVideo || reelToDelete?.type === 'user_video'
      ...
      if (isUserVideo) {
        ...
        toast({ type: 'info', message: 'AI video deleted' })
      } else {
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="hidden md:block absolute bottom-6 left-1/2 -translate-x-1/2 text-xs text-white/40">
            ...
            {(reels[currentIndex]?.isVideo || reels[currentIndex]?.isAiVideo || reels[currentIndex]?.isUserVideo) && (
              <>
