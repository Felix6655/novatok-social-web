...
import { getUploadedVideosAsync, formatDuration } from '@/lib/reels/storage'
...
/**
 * Get video reel items (async - checks Supabase first)
 * @returns {Promise<{ videoReels: Array, isSupabaseMode: boolean }>}
 */
export async function getVideoReelItemsAsync() {
  const { videos, isSupabaseMode } = await getUploadedVideosAsync()
  
  const videoReels = videos.map(video => ({
    id: video.id,
    ...
    isVideo: true,
    // For Supabase mode, signedUrl is the playback URL
    // For localStorage mode, this will be null and needs objectUrl from state
    signedUrl: video.signedUrl || null
  }))
  
  return { videoReels, isSupabaseMode }
}
 ...
 * @returns {Promise<{ reels: Array, isSupabaseMode: boolean }>}
 */
export async function getAllReelItems(limit = 50) {
  const { videoReels, isSupabaseMode } = await getVideoReelItemsAsync()
