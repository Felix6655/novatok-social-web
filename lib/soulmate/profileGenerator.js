...
// Configuration for local profile photos
const PROFILE_PHOTO_CONFIG = {
  basePath: '/profile-photos/',
  filePrefix: 'face-',
  fileExtension: '.jpg',
  totalImages: 120, // face-001.jpg through face-120.jpg
}

/**
 * Generate a deterministic hash from UUID for photo assignment
...
/**
 * Get photo URL for a profile based on UUID
 * Deterministic: same UUID always returns same photo
 * @param {string} profileId - Profile UUID
 * @returns {string} Photo URL
 */
function getProfilePhotoUrl(profileId) {
  const hash = hashUUID(profileId)
  const photoIndex = (hash % PROFILE_PHOTO_CONFIG.totalImages) + 1
  const paddedIndex = String(photoIndex).padStart(3, '0')
  return `${PROFILE_PHOTO_CONFIG.basePath}${PROFILE_PHOTO_CONFIG.filePrefix}${paddedIndex}${PROFILE_PHOTO_CONFIG.fileExtension}`
}
...
export function generateProfiles(count = 5000) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Generate photo URL deterministically from profile UUID
    // Uses local photorealistic AI-generated images from /public/profile-photos/
    const photoUrl = getProfilePhotoUrl(profileId)
